...
def solve_eig_flame_track_update(f,gas, fitting, eig2track=-1):
    N_eig = 9# selects number of eigenvalues to store for each flame location
    N_EI = 3 # number of EI species to track 
    ...
    # FORWARD FOLLOWING
    # start from position of highest eigenvalue, and track the eigenvalue in forward direction
    for loc in range(start_loc,grid_pts):
...
def solve_eig_track_no_update(f,gas, fitting, eig2track=-1):
    N_eig = 9# selects number of eigenvalues to store for each flame location
    N_EI = 3 # number of EI species to track 
